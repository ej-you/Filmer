...
type Movie struct {
	...
	// kinopoisk ID фильма
	KinopoiskID	int			`gorm:"not null;unique" json:"kinopoiskID" example:"301"`
	// название фильма
	...
	// описание фильма
	Description	string		`gorm:"null;type:TEXT" json:"description,omitempty" example:"Жизнь Томаса Андерсона разделена на две части: днём он — самый обычный офисный работник, получающий нагоняи от начальства, а ночью превращается в хакера по имени Нео, и нет места в сети, куда он бы не смог проникнуть. Но однажды всё меняется. Томас узнаёт ужасающую правду о реальности."`
	...
	UpdatedAt	time.Time	`gorm:"not null;type:TIMESTAMP" json:"updatedAt" example:"2025-02-28T22:00:05.225526936Z"`
